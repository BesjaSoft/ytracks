...
class EngineController extends BaseController {
    ...
    public function actionAutoComplete() {
        ...
        if (isset($_GET['term'])) {
            ...
            $criteria->addSearchCondition('concat(make.name,t.name)', $_GET['term']);
            ...
            foreach ($results as $result) {
                $res[] = array
                    ...
                    'value' => $result->make->name . ' ' . $result->name, // value for input field
    ...
    public function actionIndex() {
        $dataProvider = new CActiveDataProvider('Engine');
    ...
    public function actionAdmin() {
        $model = new Engine('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['Engine']))
            $model->attributes = $_GET['Engine'];
    ...
    public function loadModel($id) {
        $model = Engine::model()->findByPk((int) $id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'engine-form') {
