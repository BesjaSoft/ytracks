...
class SubroundController extends BaseController {
     ...
     */
    public function actionCreate($roundid) {
        $model = new Subround();
        if (isset( $roundid)) {
            $round = Round::model()->findByPk($roundid);
            $model->round_id = $roundid;
            $model->start_date = $round->start_date;
            $model->end_date = $round->end_date;
        }
        ...
        if (isset($_POST['Subround'])) {
            ...
            if ($model->save()) {
                $this->redirect(array('round/view', 'id' => $model->round_id));
            }
        }
