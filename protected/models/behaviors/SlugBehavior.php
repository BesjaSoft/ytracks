...
class SlugBehavior extends CActiveRecordBehavior {
    ...
    public function beforeSave($event) {
        ...
        if ($this->Owner->isNewRecord || $this->overwrite === true || empty($this->Owner->alias)) {

            $slug = $this->_getUniqueSlug();
            $this->Owner->alias = $slug;
        }
     ...
     * @param string $slug
     */
    private function _getUniqueSlug() {

        // First build the titl which is involved to build the string
        // Check if the title_col is an array:
        $title = '';
        if (is_array($this->title_col)) {
            $columns = $this->title_col;
            foreach ($columns as $column) {
                if (is_array($column)) {
                    $related = $this->owner->getRelated($column[0]);
                    $title = $title . ' ' . $related->{$column[1]};
                } else {
                    $title = $title . ' ' . $this->Owner->{$column};
                }
            }
        } else {
            $title = $this->Owner->{$this->title_col};
        }
        // then build the slug
        $slug = $this->getSlug($title);
        $this->pk_col = $this->Owner->getMetaData()->tableSchema->primaryKey;
        $this->Owner->{$this->slug_col} = $slug;
        $matches = $this->getMatches($title);
        if ($matches) {
            $ar_matches = array();
    ...
    public function getMatches($slug) {
        $slugCriteria = new CDbCriteria;
        $slugCriteria->select = "$this->slug_col, $this->pk_col";
        $slugCriteria->compare($this->slug_col, $slug, false);

        $slugs = $this->Owner->findAll($slugCriteria);
    ...
    public function getSlug($title) {
        ...
        $title = $this->_remove_accents($title);
        if ($this->_seems_utf8($title)) {
            if (function_exists('mb_strtolower')) {
                $title = mb_strtolower($title, 'UTF-8');
            }
            $title = self::_utf8_uri_encode($title, 200);
        }
     ...
     */
    private function _utf8_uri_encode($utf8_string, $length = 0) {
        $unicode = '';
        $values = array();
        $num_octets = 1;
        $unicode_length = 0;
