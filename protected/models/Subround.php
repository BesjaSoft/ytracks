 ...
 * @property integer $id
 * @property integer $projectround_id
...
class Subround extends BaseModel
{
	...
	public function rules()
	{
		...
		return array(
			array('projectround_id, subroundtype_id, ordering, created', 'required'),
			array('projectround_id, subroundtype_id, ordering, checked_out, published, deleted', 'numerical', 'integerOnly'=>true),
			array('factor', 'length', 'max'=>8),
			array('start_date, end_date, name, description, comment, modified, deleted_date', 'safe'),
			// foreign key checks:
			array('projectround_id', 'exist','attributeName'=>'id', 'className'=>'Round'),
			array('subroundtype_id', 'exist','attributeName'=>'id', 'className'=>'Subroundtype'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, projectround_id, subroundtype_id, name, ordering, start_date, end_date, description, comment, factor, checked_out, checked_out_time, published, created, modified, deleted, deleted_date', 'safe', 'on'=>'search'),
		);
	}
    ...
    public function relations()
    {
        ...
        return array('round' => array(self::BELONGS_TO, 'Round', 'projectround_id'),
	...
	public function search()
	{
		...
		$criteria->compare('projectround_id',$this->projectround_id);
    ...
    public function copyResults()
    {
        ...
        if (count($this->results) == 0) {
            $query = ' select max(sr.id) as max'
                ...
                . ' inner join jos_tracks_projects_rounds pr on pr.id = sr.projectround_id '
