...
//Default confirmSearch() will always return true
//Overwrite this function to create custom confirmSearch()
...
function getProperties(tr, id, controller, message, mode, currentTab) {
	...
	$('#properties').html('');
	$('#properties').block(blockOptions);
	
	$.post(url, queryString, function(data) {
	    $('#properties').unblock();
		$('#properties').html(data);
		$('a[id=toolTip]').Tooltip();
		if ($('div#tabsB').length && $('div#tabsB a.current:visible').length == 0) {
			activateTab($('div#tabsB li:first').attr('id'));
		}
 ...
 * Show new 
 */
function newItem(controller, initialData) {
	deleteEmptyRows();
	newEmptyLine();
 ...
 */
function saveItem(controller, action, confirmSave) 
{
    // Default value for confirmSave
    if (confirmSave != true) {
        confirmSave = false;
    }
    
    if (!confirmSave || confirmSaveItem()) {
        disableButtons();
        var rand = Math.random(9999);    
        var form = $('#edit');
        ...
        var options = {
                ...
                type: 'POST',
                data: {action: action, rand: rand},
                success: function(data){
                    handleUpdateProperties(data);
                    // EG: This is a workaround to reset the form action attribute back to before the ajaxSubmit is called.
                    // The ajaxSubmit will change the action attribute of the form into the url
                    // Because we use the submit button to submit the form, it will cause the form to be submitted twice for the second submit (the first one is fine)
                    // Instead of changing all submit button into normal button, I chose to do this workaround.
...
// Default confirmSaveItem() will always show text from $('#confirmSaveItemMessage')
// Overwrite this function to create custom confirm dialog
function confirmSaveItem()
{
    return confirm($('#confirmSaveItemMessage').val());
}
...
function showMessages(messageList, messageType) {
	if (messageList.length > 0) {
		switch(messageType) {
			case 'error':
				$('#messageBox').attr('class', 'clean-error');
				break;
			
			case '':
			case 'ok':
				$('#messageBox').attr('class', 'clean-ok');	
				break;
				
			default:
				$('#messageBox').attr('class', 'clean-yellow');	
				break;
		}
...
function sortList(controller, orderBy, filter) {
	disableButtons();
    var params = {"filter" : filter, "orderBy" : orderBy, "rand" : Math.random(9999)};
	$.post(BASE_URL+'/'+controller+'/search', params, function(data){
		$('#list').html(data);
		enableButtons();
	});
}
...
function gotoPage(page, controller, filter, orderBy) {
	...
	var params = {
	    ...
	    "orderBy" : orderBy, "rand" : Math.random(9999)
	};
	$.post(BASE_URL+'/'+controller+'/search', params, function(data) {
		$('#listLoading').hide();
		$('#list').html(data);
...
//Provide generic checkbox toggle function
function toggleCheckboxAction(checkbox, targetId, callback)
{
     // If no callback specified use the default callback instead
     if (typeof(callback) != 'function') {
         callback = onToggleCheckbox;
     }
     callback(checkbox.checked, targetId);
}

function onToggleCheckbox(value, targetId)
{
     var decoration;
     ...
     $('#'+targetId).css('text-decoration', decoration);
}