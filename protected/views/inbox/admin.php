...
foreach (Inbox::model()->getFolders() as $key => $folder)
{
    switch ($key){ 
        case Inbox::FOLDER_INBOX:
            echo "<div>".CHtml::image($themePath."/images/icons/folder.png")." ".CHtml::link($folder, array("/inbox/admin/status/".Inbox::STATUS_NEW))." (".$count_inbox.") </div>";
            break;
        case Inbox::FOLDER_SENT:
            echo "<div>".CHtml::image($themePath."/images/icons/folder.png")." ".CHtml::link($folder, array("/inbox/admin/status/".Inbox::STATUS_SENT))." (".$count_sent.") </div>";
            break;
        case Inbox::FOLDER_DRAFT:
            echo "<div>".CHtml::image($themePath."/images/icons/folder.png")." ".CHtml::link($folder, array("/inbox/admin/status/".Inbox::STATUS_DRAFT))." (".$count_draft.") </div>";
            break;
        case Inbox::FOLDER_TRASH:
            echo "<div>".CHtml::image($themePath."/images/icons/folder.png")." ".CHtml::link($folder, array("/inbox/admin/status/".Inbox::STATUS_DELETED))." (".$count_trash.") </div>";
            break;
    }
}
